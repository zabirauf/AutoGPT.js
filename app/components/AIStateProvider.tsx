...
const DEFAULT_AI_STATE = Object.freeze<AIState>({
    ...
    aiInfo: {
        ...
        description: "an AI designed to autonomously develop and run businesses with the sole goal of increasing your net worth.",
        goals: [
            "Increase net worth",
...
const AIStateDispatcherContext = createContext<AIStateDispatchers>({ setupDispatcher: () => {}, aiInfoDispatcher: () => {}});

export function AIStateProvider({ children }: PropsWithChildren<{}>) {
  const [setupState, setupDispatcher] = useReducer(setupReducer, {...DEFAULT_AI_STATE.setup});
  const [aiInfoState, aiInfoDispatcher] = useReducer(aiInfoReducer, { ...DEFAULT_AI_STATE.aiInfo });

  return (
    <AIStateContext.Provider value={{aiInfo: aiInfoState, setup: setupState }}>
      <AIStateDispatcherContext.Provider value={{aiInfoDispatcher, setupDispatcher }}>
        {children}
...
type SetupReducerAction = "next_stage" | "init_stage";

const SetupReduceNextStageMap: {[key in SetupReducerState["stage"]]: SetupReducerState["stage"]} = {
  "not_init": "get_token",
...
function setupReducer(state: SetupReducerState, action: SetupReducerAction): SetupReducerState {
...
function aiInfoReducer(state: AIInfoState, action: AIInfoReducerAction): AIInfoState {
